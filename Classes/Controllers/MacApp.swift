...
//
    ...
    open class App: NSApplication, NSApplicationDelegate {
        override public static var shared: App { super.shared as! App }
        ...
        @objc private func interfaceModeChanged() {
            theme = UserDefaults.standard.string(forKey: "AppleInterfaceStyle")?.lowercased() == "dark" ? .dark : .light
        }
        ...
        private func parseAppBuilderItem(_ item: AppBuilderItem) {
            switch item {
            case let .statusItems(values): _statusItems.append(contentsOf: values)
            case let .windows(values): _windows.append(contentsOf: values)
            case let .items(items): items.forEach { parseAppBuilderItem($0) }
            case .none: break
            }
        }
...
@resultBuilder public struct AppBuilder {
    ...
    @resultBuilder public enum AppBuilder {
        public typealias Block = () -> AppBuilderContent
